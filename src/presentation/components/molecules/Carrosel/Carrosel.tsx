...
interface Image {
  label: string;
  imgPath: string;
  altImgPath?: string; // Propriedade opcional
}

interface CarroselProps {
  images: Image[]; // Define o tipo para as imagens recebidas via props
}

export const Carrosel: React.FC<CarroselProps> = ({ images }) => {
  const theme = useTheme();
  const [activeStep, setActiveStep] = useState(0);
  const maxSteps = images.length;

  const [currentImage, setCurrentImage] = useState(images[0]?.imgPath || '');
  const [isFirstImage, setIsFirstImage] = useState(true);

  // Alternar entre as duas imagens em um intervalo de tempo (apenas para o primeiro slide)
  useEffect(() => {
    if (images.length > 0 && images[0].altImgPath) {
      const interval = setInterval(() => {
        setIsFirstImage((prev) => !prev);
      }, 400); // Intervalo de tempo
      return () => clearInterval(interval); // Limpa o intervalo quando o componente desmontar
    }
  }, [images]);
  ...
  useEffect(() => {
    if (images.length > 0 && images[0].altImgPath) {
      setCurrentImage(isFirstImage ? images[0].imgPath : images[0].altImgPath);
    }
  }, [isFirstImage, images]);
        ...
        {images.map((step, index) => (
          <div key={step.label}>
            {Math.abs(activeStep - index) <= 2 ? (
              ...
              >
                {index === 0 && step.altImgPath ? ( // Apenas para o primeiro slide com alternância de imagens
                  <Box
                    component="img"
                    sx={{
                      maxHeight: '100%', // Ajusta a imagem para caber no contêiner
